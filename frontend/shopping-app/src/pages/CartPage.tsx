...
export default function Cart() {
  ...
  if (loading) return <div className="p-10 text-center font-['Lora'] font-bold">Loading...</div>;
  ...
  return (
    <div className="bg-white lg:bg-[#F9F9F9] min-h-screen font-['Lora'] serif">
      <div className="max-w-[1200px] mx-auto bg-white min-h-screen lg:min-h-[auto] lg:mt-10 lg:mb-20 lg:rounded-[40px] lg:shadow-xl overflow-hidden relative">
        ...
        {/* --- [HEADER] ProductsList & LikesPage와 동일하게 수정된 부분 --- */}
        ...
        <div className="lg:flex lg:gap-0">
          <div className="lg:flex-[1.5] lg:p-10 lg:border-r lg:border-gray-100">
              ...
              {cartItems.length > 0 ? (
                cartItems.map((item) => (
                  <div key={item._id} className="bg-white px-6 py-5 flex gap-4 relative lg:px-0">
                    <button onClick={() => removeItem(item._id)} className="absolute top-5 right-6 lg:right-0 text-black hover:text-red-500 transition-colors">
                    ...
                    <div className="flex flex-col justify-between flex-1">
                      <div className="pr-8">
                        <h3 className="text-[15px] lg:text-[17px] font-bold leading-snug text-black mb-1">{item.productId?.name}</h3>
                        <div className="flex flex-col">
                          {item.productId?.sale && <span className="text-[12px] text-[#A1A1A1] line-through">${item.productId.sale.original.toFixed(2)}</span>}
                          <span className="text-[16px] lg:text-[18px] font-black text-[#D3180C]">${item.productId?.price.toFixed(2)}</span>
                        </div>
                      </div>
                      <div className="flex items-center bg-white rounded-full px-2 py-0.5 gap-4 w-fit border border-[#EEEEEE] self-end lg:self-start shadow-sm lg:mt-2">
                        <button onClick={() => updateQty(item._id, item.quantity - 1)} className="w-8 h-8 flex items-center justify-center text-[#8E8E93] text-xl font-light hover:text-black">-</button>
                        <span className="font-bold text-[14px]">{item.quantity}</span>
                        <button onClick={() => updateQty(item._id, item.quantity + 1)} className="w-8 h-8 flex items-center justify-center text-black text-xl font-light hover:scale-110">+</button>
                      </div>
                    </div>
                  </div>
                ))
            ...
            <div className="lg:hidden px-8 py-8 space-y-4">
              <div className="flex justify-between text-[#8E8E93] text-[15px]"><span>Total Price</span><span className="text-black font-medium">${originalTotal.toFixed(2)}</span></div>
              <div className="flex justify-between text-[#8E8E93] text-[15px]"><span>Discount</span><span className="text-red-500">-${discount.toFixed(2)}</span></div>
              <div className="flex justify-between text-[#8E8E93] text-[15px]"><span>Estimated delivery fees</span><span className="text-black font-medium">Free</span></div>
            </div>
        ...
        <div className="lg:hidden fixed bottom-[65px] left-0 right-0 bg-[#F2F2F2] px-6 pt-6 pb-6 rounded-t-[35px] shadow-[0_-15px_30px_rgba(0,0,0,0.08)] z-40">
          <div className="flex justify-between items-center mb-5 px-1">
            <div>
              ...
              <span className="text-[#8E8E93] text-[12px] block mt-0.5">Saving Applied: ${discount.toFixed(2)}</span>
            </div>
            <span className="text-[24px] font-black text-black">${subTotal.toFixed(2)}</span>
          </div>
          <button 
            onClick={() => navigate("/checkout")}
