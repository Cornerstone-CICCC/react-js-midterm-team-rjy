    ...
    <div className="bg-white min-h-screen pb-20 font-['Lora'] serif overflow-x-hidden">
      ...
      <div className="max-w-[1200px] mx-auto">
        ...
        <header className="relative flex items-center justify-center px-6 py-6 lg:py-10 bg-white">
          <h1 className="text-[24px] lg:text-[32px] font-black tracking-tighter text-black uppercase">
            BOUTIQUE
          </h1>
          <button
          ...
          <div className="relative h-full flex items-center justify-center px-10">
            <div
              onClick={() => {
                const targetId =
                  products.length > 0
                    ? products[currentSlide % products.length]._id
                    : "1";
            ...
            <button
              onClick={prevSlide}
              className="absolute left-4 lg:left-12 z-20 text-black/10 hover:text-black"
            ...
            <button
              onClick={nextSlide}
              className="absolute right-4 lg:right-12 z-20 text-black/10 hover:text-black"
        ...
        <div className="px-6 lg:px-10 mt-10 lg:mt-16 grid grid-cols-2 lg:grid-cols-4 gap-x-5 lg:gap-x-8 gap-y-12 lg:gap-y-20">
          {products.map((product) => (
            ...
            >
              ...
              <div className="text-[16px] lg:text-[19px] font-black text-black mb-1">
                {Number(product.price).toFixed(2)}$
              </div>
              <div className="text-[13px] lg:text-[15px] text-gray-500 font-medium line-clamp-1">
                {product.name}
              </div>
            </div>
          ))}
